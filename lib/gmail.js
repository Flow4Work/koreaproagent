...
<div style="margin:19px 0 0 0;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.65;font-weight:400;color:#6b7280;">
  ...
  <div style="margin:0;">Instagram · <a href="https://www.instagram.com/notyourflavor/" style="color:#6b7280;text-decoration:underline;">@notyourflavor</a> / <a href="https://www.instagram.com/timesewingmachine" style="color:#6b7280;text-decoration:underline;">@timesewingmachine</a></div>
...
export function buildRawMessage({ to, subject, body, html, senderEmail, senderName }) {
  ...
  const suppliedHtml = String(html || '').replace(/\0/g, '').trim();
  const renderedHtml = suppliedHtml || htmlEmail(body);
  const htmlBase64 = wrapBase64(Buffer.from(renderedHtml, 'utf8').toString('base64'));
