...
export function gmailConfig(request) {
  ...
  return {
    ...
    redirectUri: env('GMAIL_OAUTH_REDIRECT_URI') || (requestOrigin ? `${requestOrigin}/api/email/oauth-callback` : ''),
    senderEmail,
...
export function buildRawMessage({ to, subject, body, senderEmail, senderName }) {
  ...
  const bodyBase64 = wrapBase64(Buffer.from(String(body || ''), 'utf8').toString('base64'));
  const message = [
