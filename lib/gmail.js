...
function env(name) {
  return clean(process.env[name] || '', 5000);
}
...
export function sessionCookie(session, secret) {
  return cookie(SESSION_COOKIE, encryptPayload(session, secret), SESSION_MAX_AGE);
}

export function stateCookie(state, secret) {
  return cookie(STATE_COOKIE, encryptPayload(state, secret), STATE_MAX_AGE);
}
...
export function buildEmailHtml(body = '') {
  ...
  const paragraphs = normalized
    ...
    .map((paragraph, index, list) => {
      ...
      const margin = index === list.length - 1 ? '0' : '0 0 16px 0';
      return `<p style="margin:${margin};font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.6;color:#111827;">${content}</p>`;
    })
    .join('');
...
<tr><td style="padding:0;font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.6;color:#111827;text-align:left;">
${paragraphs}
</td></tr>
</table>
</td></tr>
...
export function buildRawMessage({ to, subject, body, senderEmail, senderName }) {
  const cleanSubject = String(subject || '').replace(/[\r\n]+/g, ' ').trim();
  const cleanName = String(senderName || '').replace(/[\r\n<>]+/g, ' ').trim();
  const boundary = `kpa_${crypto.randomBytes(12).toString('hex')}`;
  const plainBase64 = wrapBase64(Buffer.from(String(body || ''), 'utf8').toString('base64'));
  const htmlBase64 = wrapBase64(Buffer.from(buildEmailHtml(body), 'utf8').toString('base64'));
  const message = [
