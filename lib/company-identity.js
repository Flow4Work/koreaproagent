...
function extractIdentityPage(html = '', pageUrl = '') {
  ...
  brandDeclarations(pageText(html)).forEach(value => add('brand_declaration', value, 122, 'strong'));
...
async function fetchText(url, timeoutMs = 6500) {
  ...
  try {
    const response = await fetch(url, {
      ...
      redirect: 'follow',
      cache: 'no-store',
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; KoreaAgentCompanyIdentity/3.0)' }
    });
