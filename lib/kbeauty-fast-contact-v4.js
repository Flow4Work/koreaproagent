...
function disableHunterIfLimited(result) {
  const error = clean(result?.error || result?.diagnostic?.error,120).toLowerCase();
  if (Number(result?.status || result?.diagnostic?.status) === 429 || /credit|limit|rate/.test(error)) {
    hunterDisabledUntil = Date.now() + 12 * 60 * 60 * 1000;
  }
}
...
async function hunterCompanySearch(company) {
  ...
  if(Date.now()<hunterDisabledUntil)return{domain:'',contacts:[],diagnostic:diagnostic('hunter',{error:'credit_limited',stage:'company_search'})};
  ...
  if(!result.ok){disableHunterIfLimited(result);return{domain:'',contacts:[],diagnostic:result.diagnostic};}
...
async function hunterContacts(domain) {
  ...
  if(Date.now()<hunterDisabledUntil)return{contacts:[],diagnostic:diagnostic('hunter',{error:'credit_limited',stage:'domain_search'})};
  ...
  if(!result.ok){disableHunterIfLimited(result);return{contacts:[],diagnostic:result.diagnostic};}
...
async function resolveWithExa(company,country,exaKey) {
  ...
  if(!key)return{domain:'',diagnostic:diagnostic('exa',{error:'not_configured',stage:'domain_resolution'})};
  const result=await fetchJsonResult(EXA_URL,{method:'POST',headers:{'x-api-key':key,'Content-Type':'application/json'},body:JSON.stringify({query:`${clean(company,160)} official website contact ${clean(country,80)}`,type:'fast',numResults:8,excludeDomains:[...BLOCKED]})},8500,'exa','domain_resolution');
