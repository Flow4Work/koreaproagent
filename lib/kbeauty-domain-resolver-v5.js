...
export async function resolveKBeautyDomainsV5(items = [], exaKey = '') {
  ...
  const results = await mapLimit(rows,6,async item => {
    ...
    const profile = await officialProfileResolve(item);
    if (profile.domain) return {id,company,domain:profile.domain,url:`https://${profile.domain}/`,resolvedBy:'official_profile',diagnostics:[profile.diagnostic]};
