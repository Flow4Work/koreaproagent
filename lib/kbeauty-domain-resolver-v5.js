...
const INTERCHARM_HOST = 'ick.intercharmkorea.com';
...
async function fetchText(url, timeoutMs = 7000) {
  ...
  try {
    const response = await fetch(url, {
      ...
      redirect:'follow',
      cache:'no-store',
      headers:{'User-Agent':'Mozilla/5.0 (compatible; KoreaProAgent/1.0)','Accept':'text/html,application/xhtml+xml,text/plain;q=0.9,*/*;q=0.5'}
    });
    if (!response.ok) return {ok:false,status:response.status,text:''};
    return {ok:true,status:response.status,text:(await response.text()).slice(0,450000)};
  } catch (error) {
    return {ok:false,status:0,text:'',error:error?.name==='AbortError'?'timeout':'network_error'};
  } finally { clearTimeout(timer); }
}

function profileWebsiteCandidates(html = '') {
  const raw = String(html || '').replace(/&amp;/gi,'&').replace(/&quot;/gi,'"');
  const candidates = [];
  const labelled = raw.match(/(?:Website|홈페이지)[\s\S]{0,1000}?((?:https?:\/\/|www\.)[^\s"'<>]+)/i)?.[1];
  if (labelled) candidates.push(labelled);
  const href = /href\s*=\s*["']((?:https?:\/\/|\/\/|www\.)[^"']+)["']/gi;
  let match;
  while ((match = href.exec(raw))) candidates.push(match[1]);
  return [...new Set(candidates.map(value=>String(value||'').replace(/^\/\//,'https://').replace(/[),.;]+$/,'')))];
}

async function officialProfileResolve(item = {}) {
  const sourceUrl = clean(item?.source_url,700);
  if (!sourceUrl || rootHost(sourceUrl) !== INTERCHARM_HOST || !/\/exhibitor\/exhi_detail04\.asp/i.test(sourceUrl)) {
    return {domain:'',diagnostic:diagnostic('official_profile',{error:'not_available'})};
  }
  const page = await fetchText(sourceUrl,7000);
  if (!page.ok) return {domain:'',diagnostic:diagnostic('official_profile',{status:page.status,error:page.error||'fetch_failed'})};
  for (const candidate of profileWebsiteCandidates(page.text)) {
    const domain = rootHost(candidate);
    if (!domain || domain === INTERCHARM_HOST || domain.endsWith('intercharmkorea.com') || !usableDomain(domain,item?.company||'')) continue;
    return {domain,diagnostic:diagnostic('official_profile',{ok:true,status:page.status,detail:'matched'})};
  }
  return {domain:'',diagnostic:diagnostic('official_profile',{ok:true,status:page.status,error:'no_website'})};
}
...
function domainCandidates(rows = []) {
  const seen = new Set();
  const out = [];
  for (const row of rows) {
    const domain = rootHost(row?.domain || '');
    const source = clean(row?.source,60);
    if (!domain || seen.has(domain)) continue;
    seen.add(domain);
    out.push({domain,source});
  }
...
export async function resolveKBeautyDomainsV5(items = [], exaKey = '') {
  ...
  const results = await mapLimit(rows,6,async item => {
    ...
    if (usableDomain(existing,company)) return {
      id,company,domain:existing,url:`https://${existing}/`,resolvedBy:'existing',
      domain_candidates:[{domain:existing,source:'existing'}],diagnostics:[diagnostic('existing',{ok:true,detail:existing})]
    };

    const hinted = KNOWN.get(companyKey(company));
    if (hinted) return {
      id,company,domain:hinted,url:`https://${hinted}/`,resolvedBy:'verified_domain_hint',
      domain_candidates:[{domain:hinted,source:'verified_domain_hint'}],diagnostics:[diagnostic('domain_hint',{ok:true,detail:hinted})]
    };

    const [profile,exa,tavily] = await Promise.all([
      officialProfileResolve(item),
      exaResolve(company,country,exaKey),
      tavilyResolve(company,country)
    ]);
    let candidates = domainCandidates([
      {domain:profile.domain,source:'official_profile'},
      {domain:exa.domain,source:'exa'},
      {domain:tavily.domain,source:'tavily'}
    ]);
    let hunter = {domain:'',diagnostic:diagnostic('hunter',{error:'not_needed'})};
    if (!candidates.length) {
      hunter = await hunterResolve(company);
      candidates = domainCandidates([{domain:hunter.domain,source:'hunter'}]);
    }
    const picked = candidates[0] || {domain:'',source:''};
    return {
      id,company,domain:picked.domain,url:picked.domain?`https://${picked.domain}/`:'',resolvedBy:picked.source,
      domain_candidates:candidates,
      diagnostics:[profile.diagnostic,exa.diagnostic,tavily.diagnostic,hunter.diagnostic]
    };
  });
