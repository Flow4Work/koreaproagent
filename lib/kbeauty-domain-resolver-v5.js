...
export async function resolveKBeautyDomainsV5(items = [], exaKey = '') {
  ...
  const results = await mapLimit(rows,6,async item => {
    ...
    // Broad, reusable providers contribute in parallel. None of them gates the others.
    ...
    let picked = profile.domain ? ['official_profile',profile.domain] : exa.domain ? ['exa',exa.domain] : tavily.domain ? ['tavily',tavily.domain] : ['', ''];
    // Hunter is deliberately scarce-credit recovery, not a gate in front of the broad providers.
    if (!picked[1]) {
