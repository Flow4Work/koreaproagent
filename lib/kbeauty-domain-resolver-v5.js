...
async function fetchJson(url, options = {}, timeoutMs = 8500, provider = 'provider') {
  ...
  try {
    ...
    if (!response.ok) return {ok:false,data:null,diagnostic:diagnostic(provider,{status:response.status,error:clean(data?.error||data?.message||`http_${response.status}`,100)})};
...
export async function resolveKBeautyDomainsV5(items = [], exaKey = '') {
  ...
  const results = await mapLimit(rows,3,async item => {
    ...
    const [hunter,exa,tavily] = await Promise.all([
      hunterResolve(company),
      exaResolve(company,country,exaKey),
      tavilyResolve(company,country)
    ]);
    const picked = hunter.domain ? ['hunter',hunter.domain] : exa.domain ? ['exa',exa.domain] : tavily.domain ? ['tavily',tavily.domain] : ['', ''];
    return {
      id,company,domain:picked[1],url:picked[1]?`https://${picked[1]}/`:'',resolvedBy:picked[0],
      diagnostics:[hunter.diagnostic,exa.diagnostic,tavily.diagnostic]
    };
  });
