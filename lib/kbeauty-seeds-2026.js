...
export async function collectOfficial2026Seeds(target = TARGET_DEFAULT) {
  ...
  // InterCHARM's current page is explicitly dated July 1-3, 2026. Use direct HTML plus JS-capable Tavily extraction/crawl.
  const intercharmHtml = await fetchText(INTERCHARM_MODERN);
  addMany(parseGenericOfficialDirectory(intercharmHtml,{pageUrl:INTERCHARM_MODERN,event:'InterCHARM Korea 2026',score:92}));
  const extracted = await tavilyAdvancedExtract([INTERCHARM_MODERN]);
  ...
  // in-cosmetics has already rolled the live directory forward to 2027, so only exact indexed 2026 exhibitor-detail evidence is accepted.
  addMany(await tavilyIncosmetics2026Search());
