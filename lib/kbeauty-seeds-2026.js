...
function validCompany(value = '') {
  ...
  if (/^(?:view profile|profile|home|about|contact|search|more|next|previous|exhibitor list|website|products?|company information)$/i.test(name)) return false;
  if (/^(?:booth(?: no\.)?|skin care|body care|cosmetics|manufacturing)$/i.test(name)) return false;
  return true;
}
...
function companyFromDetailBlock(block = '', anchorText = '') {
  const candidates = [stripTags(anchorText)];
  const heading = /<h[1-6]\b[^>]*>([\s\S]*?)<\/h[1-6]>/gi;
  let match;
  while ((match = heading.exec(block))) candidates.push(stripTags(match[1]));
  const attrs = /\b(?:title|alt|data-company(?:-name)?)\s*=\s*["']([^"']+)["']/gi;
  while ((match = attrs.exec(block))) candidates.push(stripTags(match[1]));
  return candidates.find(validCompany) || '';
}
...
export function parseIntercharmList(html = '', pageUrl = INTERCHARM_BASE) {
  ...
  for (let index = 0; index < matches.length; index += 1) {
    ...
    const end = index + 1 < matches.length ? Number(matches[index + 1].index) || raw.length : Math.min(raw.length, start + 5000);
    const block = raw.slice(start, end);
    const company = companyFromDetailBlock(block, match[2]);
    ...
    rows.push({
      ...
      score: 88,
      source_event: 'InterCHARM Korea 2026',
      source_url: absoluteUrl(match[1]) || pageUrl,
