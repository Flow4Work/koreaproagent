...
const CURATED_FOREIGN_2026 = [
  ...
  },
  {
    ...
    evidence_type: 'official_exhibitor_profile', evidence_text: 'Official InterCHARM Korea 2026 exhibitor profile; booth I43.'
  },
  {
    company: 'Daxal Cosmetics Pvt Ltd', country: 'India', domain: 'daxalcosmetics.com', tier: 'korea_beauty_event_2026', score: 96,
    source_event: 'InterCHARM Korea 2026',
    source_url: 'https://ick.intercharmkorea.com/eng/exhibitor/exhi_detail04.asp?idx=6535&param=%26page%3D1&ref_idx=17926',
    evidence_type: 'official_exhibitor_profile', evidence_text: 'Current InterCHARM Korea exhibitor evidence; official company domain independently verified.'
  },
  {
    ...
    evidence_type: 'official_exhibitor_directory', evidence_text: 'Current InterCHARM Korea exhibitor source; official company domain independently verified.'
  },
  {
...
function validCompany(value = '') {
  ...
  if (/^(?:view profile|profile|home|about|contact|search|more|next|previous|exhibitor list|exhibitor directory|website|email|products?|documents?|company information|brands?|categories?|filters?|features?|show information|useful links)$/i.test(name)) return false;
...
function rowForCompany(company, block, { pageUrl = '', event = '', score = 90 } = {}) {
  ...
  return {
    ...
    source_event:event,
    source_url:pageUrl,
    source_title:`${event} official 2026 exhibitor directory`,
    evidence_type:'official_exhibitor_directory',
    evidence_text:`Listed in the official ${event} 2026 exhibitor directory.`,
    curated_2026:true,
...
export function parseGenericOfficialDirectory(html = '', { pageUrl = '', event = '', score = 90 } = {}) {
  ...
  for (let index = 0; index < matches.length; index += 1) {
    ...
    const company = stripTags(match[2], 180);
    ...
    seen.add(key);
    rows.push(rowForCompany(company, block, { pageUrl, event, score }));
  }
  return rows;
}
...
export function parseMarkdownOfficialDirectory(markdown = '', { pageUrl = '', event = '', score = 90 } = {}) {
  ...
  for (let index = 0; index < matches.length; index += 1) {
    ...
    const company = stripMarkdown(match[2], 180);
    const key = companyKey(company);
    if (!validCompany(company) || !key || seen.has(key)) continue;
    let end = raw.length;
    for (let next = index + 1; next < matches.length; next += 1) {
...
async function fetchText(url, timeoutMs = 8000) {
  ...
  try {
    const response = await fetch(url, {
      method:'GET',
      headers:{'User-Agent':'Mozilla/5.0 (compatible; KoreaProAgent/1.0)','Accept':'text/html,application/xhtml+xml'},
...
async function tavilyAdvancedExtract(urls = []) {
  ...
  try {
    const response = await fetch(TAVILY_EXTRACT_URL, {
      ...
      body:JSON.stringify({urls,extract_depth:'advanced',format:'markdown',include_images:false}),
...
async function tavilyDirectoryCrawl(url = '', limit = 80) {
  const key = clean(process.env.TAVILY_API_KEY, 5000);
  if (!key || !url) return [];
  const controller = new AbortController();
  ...
  try {
    const response = await fetch(TAVILY_CRAWL_URL, {
      ...
      body:JSON.stringify({
        url,
        instructions:'Follow only current 2026 exhibitor profile or exhibitor detail pages. Ignore navigation, news, registration, venue and privacy pages.',
        max_depth:1,max_breadth:Math.min(150,limit),limit,
        extract_depth:'basic',format:'markdown',allow_external:false
      }),
      cache:'no-store',signal:controller.signal
    });
    if (!response.ok) return [];
...
export async function collectOfficial2026Seeds(target = TARGET_DEFAULT) {
  ...
  const sources = [
    {url:INTERCHARM_MODERN,event:'InterCHARM Korea 2026',score:92},
    {url:INCOSMETICS_2026,event:'in-cosmetics Korea 2026',score:90}
  ];

  const direct = await Promise.all(sources.map(source => fetchText(source.url)));
  direct.forEach((html,index) => addMany(parseGenericOfficialDirectory(html,{pageUrl:sources[index].url,event:sources[index].event,score:sources[index].score})));

  const extracted = await tavilyAdvancedExtract(sources.map(source => source.url));
  for (const result of extracted) {
    const source = sources.find(item => rootHost(item.url) === rootHost(result?.url)) || sources[0];
    addMany(parseMarkdownOfficialDirectory(result?.raw_content || '',{pageUrl:result?.url || source.url,event:source.event,score:source.score}));
  }

  if (rows.length < Math.min(100,wanted)) {
    const crawled = await tavilyDirectoryCrawl(INTERCHARM_MODERN, Math.min(100,wanted));
  ...
  if (rows.length < Math.min(180,wanted)) {
    const crawled = await tavilyDirectoryCrawl(INCOSMETICS_2026, Math.min(100,wanted));
    for (const result of crawled) addMany(parseMarkdownOfficialDirectory(result?.raw_content || '',{pageUrl:result?.url || INCOSMETICS_2026,event:'in-cosmetics Korea 2026',score:90}));
  }
