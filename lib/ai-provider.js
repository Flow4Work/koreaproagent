...
export async function chatJson({ prompt, maxTokens = 1800, timeoutMs = 35000, temperature = 0 }) {
  ...
  // DeepSeek V4 Flash reasons before it writes the final answer. A small output budget
  // can therefore end with content=null even when the API call itself succeeded.
  const budgets = [Math.max(8192, maxTokens), Math.max(16384, maxTokens * 2)];
