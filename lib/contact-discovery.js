...
const GENERIC_LOCAL_PARTS = new Set([
  ...
  'office','partners','partnerships','press','privacy','sales','security','support','team','business','events','community'
]);
const JUNK_LOCAL_PARTS = new Set(['billing','careers','hr','jobs','legal','privacy','security','noreply','no-reply','donotreply']);
...
function usefulPublicEmail(email = '') {
  const local = localPart(email);
  return Boolean(local && !JUNK_LOCAL_PARTS.has(local));
}

function genericEmailScore(email = '') {
  const local = localPart(email);
  if (/^(partnerships?|partners?|business|bizdev|bd|sales|events?)$/.test(local)) return 82;
  if (/^(community|marketing|media|press|team)$/.test(local)) return 76;
  if (/^(hello|contact|info|office)$/.test(local)) return 70;
  if (/^(help|support)$/.test(local)) return 58;
  return personalEmail(email) ? 84 : 64;
}
...
function roleScore(title = '', recommendedRole = '') {
  ...
  if (/business development|partnership|growth|sales|revenue|commercial|go-to-market|gtm|operations|events|community|marketing/.test(text)) score += 26;
  return score;
}
...
function dedupe(contacts = []) {
  ...
  return contacts.filter(contact => {
    const emailKey = clean(contact.email, 240).toLowerCase();
    const key = emailKey || `${contact.linkedin_url || ''}|${contact.first_name || ''}|${contact.last_name || ''}`;
...
async function fetchHtml(url, timeoutMs = 3500) {
  ...
  try {
    const response = await fetch(url, {
      ...
      redirect: 'follow',
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; KoreaProAgent/1.1; public-contact-discovery)' }
    });
...
function extractEmails(text = '', domain = '') {
  ...
  return [...new Set(matches.map(v => v.toLowerCase()))]
    .filter(email => sameDomain(email, domain) && usefulPublicEmail(email));
}
...
function contactLinks(html = '', baseUrl = '', domain = '') {
  ...
  while ((match = re.exec(String(html))) && out.length < 10) {
    try {
      ...
      if (!/(about|team|leadership|company|contact|people|management|press|media|partner|partnership|business|events|community|marketing)/i.test(url.pathname)) continue;
      out.push(url.href);
    } catch { }
  }
...
function publicContactsFromPage(html = '', domain = '', pageUrl = '') {
  return extractEmails(html, domain).map(email => toRawContact({
    email,
    sources: [pageUrl],
    provider: 'public_web',
    score: genericEmailScore(email)
  }));
}
...
async function publicWebsiteSearch(domain) {
  ...
  let contacts = publicContactsFromPage(html, domain, homepage);
  const links = contactLinks(html, homepage, domain);
  const common = ['/contact','/about','/team','/press','/partnerships'];
  const urls = [...new Set([
    ...links,
    ...common.map(path => new URL(path, homepage).href)
  ])].slice(0, 5);

  if (contacts.length < 4 && urls.length) {
    const pages = await Promise.all(urls.map(async url => ({ url, html: await fetchHtml(url) })));
    for (const page of pages) {
      if (!page.html) continue;
      contacts.push(...publicContactsFromPage(page.html, domain, page.url));
      if (dedupe(contacts).length >= 6) break;
    }
  }

  return dedupe(contacts).slice(0, 8);
}
...
async function apolloSearch(domain, recommendedRole) {
  ...
  appendParams(params, 'person_titles[]', [recommendedRole, 'Events', 'Operations', 'Partnerships', 'Community', 'Marketing', 'Founder', 'CEO']);
...
export async function findContacts(domainOrUrl, options = {}) {
  ...
  const maxContacts = Math.min(Math.max(Number(options.maxContacts) || 8, 1), 10);
  const minContacts = Math.min(Math.max(Number(options.minContacts) || 4, 1), maxContacts);
  const cacheKey = `${domain}|${recommendedRole.toLowerCase()}|${maxContacts}|${minContacts}`;
  ...
  for (const [provider, run, enabled] of steps) {
    ...
    try {
      const found = dedupe(await run());
      if (found.length) {
        providers.push(provider);
        collected = dedupe([...collected, ...found]);
        attempts.push({ provider, status: 'found', count: found.length });
        if (collected.length >= minContacts) break;
      } else {
  ...
  const value = {
    emails: collected.slice(0, maxContacts),
    company: null,
    provider: [...new Set(providers)].join('+') || null,
    attempts
  };
  cache.set(cacheKey, { expiresAt: Date.now() + (value.emails.length ? HIT_TTL_MS : MISS_TTL_MS), value });
  return value;
}
