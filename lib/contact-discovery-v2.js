...
export async function findContacts(domainOrUrl, options = {}) {
  ...
  const configuredProviderCount = Object.values(status).filter(Boolean).length;
  const baseTargetCount = 10 + configuredProviderCount;
  const [baseResult, jinaResult, hunterResult] = await Promise.all([
    findBaseContacts(domain, { maxContacts: baseTargetCount, minContacts: baseTargetCount, recommendedRole }).catch(error => ({ emails: [], attempts: [{ provider: 'base', status: 'error', code: Number(error?.status) || null }] })),
