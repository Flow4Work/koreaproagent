...
function remainingFromHeaders(headers) {
  return {
    dailyLeft: Number(headers?.get?.('x-daily-request-left')),
    minuteLeft: Number(headers?.get?.('x-minute-request-left')),
    dailyReset: Number(headers?.get?.('x-daily-reset-seconds'))
  };
}
...
function shapeContact(match = {}, expectedDomain = '') {
  ...
  const companyDomain = rootHost(company?.domain || company?.website || expectedDomain);
  if (!validEmail(email) || status !== 'VERIFIED' || JUNK.test(localPart(email))) return null;
  if (!sameDomain(email, expectedDomain) && !sameDomain(email, companyDomain)) return null;
...
async function recoverOne(row = {}) {
  ...
  const enrich = await requestJson(ENRICH_URL, {
    ...
    data:candidates.map((item,index) => ({
      ...
      person_id:clean(item?.person?.person_id, 120)
    })).filter(item => item.person_id)
  }, 'person_enrich');
  diagnostics.push(enrich.diagnostic);
  if (!enrich.ok) return { ...row, diagnostics };

  const contacts = (Array.isArray(enrich.data?.matched) ? enrich.data.matched : [])
    .map(match => shapeContact(match, domain))
    .filter(Boolean);
  if (!contacts.length) return { ...row, diagnostics:[...diagnostics, diagnostic({ ok:true, status:enrich.status, error:'no_verified_email', stage:'person_enrich' })] };
  ...
  for (const contact of [...existing, ...contacts]) {
    ...
    dedup.set(email, contact);
  }
  return {
    ...row,
    contacts:[...dedup.values()].slice(0, 6),
    status:'found',
    diagnostics:[...diagnostics, diagnostic({ ok:true, status:enrich.status, stage:'person_enrich', detail:`verified:${contacts.length}` })]
  };
}
