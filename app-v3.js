...
function toolKeys() { return { jinaKey:localStorage.getItem(JINA_KEY) || '', braveKey:localStorage.getItem(BRAVE_KEY) || '', dartKey:localStorage.getItem(DART_KEY) || '' }; }
...
function leadReady(lead) { return Boolean(lead?.contact?.email && (lead.message_ko || lead.message_en)); }
function leadMessage(lead) { const cfg = CAMPAIGNS[lead.campaign] || CAMPAIGNS.kbw; return cfg.message === 'en' ? (lead.message_en || '') : (lead.message_ko || lead.message_en || ''); }
function gmailUrl(lead) { const email = lead?.contact?.email, subject = clean(lead.subject,180), body = leadMessage(lead); if (!email || !subject || !body) return ''; return `https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent(email)}&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`; }
function stageText(lead) { if (leadReady(lead)) return '발송 준비'; if (lead.contact_status === 'searching') return '이메일 찾는 중'; if (lead.contact_status === 'failed') return '이메일 미확보'; return lead.verified_company ? '검증 후보' : '후보'; }
function remainingText() { if (!state.auto || !state.autoUntil) return ''; const left = Math.max(0,state.autoUntil-Date.now()); return `${Math.floor(left/60000)}:${String(Math.floor((left%60000)/1000)).padStart(2,'0')}`; }
...
function updateMainButton() {
  ...
  if (state.manualRunning) { b.textContent = '첫 사냥 중…'; b.disabled = true; return; }
...
async function manualHunt() {
  if (state.manualRunning || state.auto) return; state.manualRunning = true; updateMainButton();
...
async function startAutoHunt() { if(state.auto)return; state.auto=true; state.autoUntil=Date.now()+AUTO_DURATION_MS; state.statusText='15분 자동사냥 시작'; render(); while(state.auto&&Date.now()<state.autoUntil){ try{await runHuntCycle();}catch(e){if(!state.auto)break;state.statusText=`이번 회차 실패 · ${clean(e.message,100)}`;renderSummary();} if(!state.auto||Date.now()>=state.autoUntil)break; await waitRandom(); } if(state.auto){state.auto=false;state.statusText='15분 자동사냥 완료';render();} }
function stopAutoHunt(){state.auto=false;state.autoUntil=0;abortAll();state.statusText='자동사냥 중지 · 현재 결과 유지';render();}
function handleRunButton(){if(state.auto)return stopAutoHunt();if(state.firstRun)return startAutoHunt();return manualHunt();}
...
function setupSettings(){
  const fields=[[EXA_KEY,'exaKey'],[JINA_KEY,'jinaKey'],[BRAVE_KEY,'braveKey'],[DART_KEY,'dartKey']];
  for(const [key,id] of fields) $(id).value=localStorage.getItem(key)||'';
  const note=()=>{const active=[];if(localStorage.getItem(JINA_KEY))active.push('Jina');if(localStorage.getItem(BRAVE_KEY))active.push('Brave');if(localStorage.getItem(DART_KEY))active.push('OpenDART');$('searchSettingsNote').textContent=active.length?`${active.join(' + ')} 활성 · Exa 미사용`:'Tavily만 사용 · Exa 미사용';};
