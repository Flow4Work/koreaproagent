...
function rateInfo(response){return{remaining_requests:response.headers.get('x-ratelimit-remaining-requests'),remaining_tokens:response.headers.get('x-ratelimit-remaining-tokens'),reset_requests:response.headers.get('x-ratelimit-reset-requests'),reset_tokens:response.headers.get('x-ratelimit-reset-tokens'),retry_after:response.headers.get('retry-after')}}
...
async function callGroq({model,prompt,timeoutMs}){
  ...
  try{
    const response=await fetch(GROQ_URL,{method:'POST',headers:{Authorization:`Bearer ${process.env.GROQ_API_KEY}`,'Content-Type':'application/json','Groq-Model-Version':GROQ_VERSION},body:JSON.stringify({model,messages:[{role:'system',content:'Evidence-first Korea GTM researcher. Use current web evidence. Never invent facts. Return one JSON object only.'},{role:'user',content:prompt}],temperature:0.1,response_format:{type:'json_object'},citation_options:'disabled',compound_custom:{tools:{enabled_tools:['web_search']}}}),signal:controller.signal});
    const raw=await response.text();if(!response.ok){let detail=raw;try{detail=JSON.parse(raw)?.error?.message||raw}catch{}const e=new Error(`Groq HTTP ${response.status}: ${safeError(detail)}`);e.status=response.status;e.retryAfter=response.headers.get('retry-after');e.rate=rateInfo(response);throw e;}
    const payload=JSON.parse(raw);const message=payload?.choices?.[0]?.message;return{parsed:parseMaybeJson(message?.content),payload,toolCalls:Array.isArray(message?.executed_tools)?message.executed_tools.length:0,durationMs:Date.now()-started,rate:rateInfo(response)};
  }catch(error){if(error?.name==='AbortError'){const e=new Error(`Groq research timed out after ${Math.round(timeoutMs/1000)}s`);e.status=504;throw e;}throw error;}finally{clearTimeout(timer)}
}
...
export async function POST(request){
  ...
  const count=clampInt(body.count,3,8,5);const mode=body.mode==='deep'?'deep':'fast';const productHint=cleanText(body.productHint,1600);const targetNotes=cleanText(body.targetNotes,1800);const seeds=cleanText(body.seeds,3500);
  const prompt=`MISSION\nResearch the SaaS below and create a Korea Prospect Pack with up to ${count} real Korean companies that plausibly need it.\n\nCLIENT URL\n${clientUrl}\n\nPRODUCT HINT\n${productHint||'Infer carefully from the public product information and search results.'}\n\nTARGET NOTES\n${targetNotes||'Choose the strongest Korea ICP based on the product.'}\n\nOPTIONAL SEEDS\n${seeds||'None. Discover candidates yourself.'}\n\nRULES\n1. Use current web search for the SaaS and Korean prospects.\n2. Prefer hiring, expansion, product launches, support load, partnerships, funding, regulation, or technology change relevant to the product.\n3. Every non-obvious claim needs source_urls. Never invent URLs, people, dates, hiring, funding, or customer counts.\n4. Fill contact_name/title/profile only when publicly verified. Otherwise leave them blank and provide recommended_role + contact_search_query. Never guess email addresses.\n5. Fit score = 40 use-case fit + 30 current signal + 20 reachable buyer + 10 evidence.\n6. Korean and English outreach must be short, specific, and evidence-based.\n\nOUTPUT ONLY JSON\n{"client":{"name":"","url":"","product":"","korea_value_proposition":""},"icp":{"summary":"","industries":[],"company_signals":[],"buyer_roles":[]},"prospects":[{"company":"","url":"","industry":"","fit_score":0,"why_fit":"","buying_signal":"","signal_date":"","source_urls":[],"contact_name":"","contact_title":"","contact_profile_url":"","recommended_role":"","contact_search_query":"","sales_angle":"","message_ko":"","message_en":"","confidence":"high|medium|low","warning":""}],"strategy":{"first_segment":"","core_offer":"","outreach_sequence":[],"next_action":""},"research_notes":[]}`;
  const attempts=mode==='deep'?[{model:'groq/compound',timeoutMs:26000},{model:'groq/compound-mini',timeoutMs:15000}]:[{model:'groq/compound-mini',timeoutMs:18000},{model:'groq/compound',timeoutMs:22000}];
  const failures=[];
  for(let i=0;i<attempts.length;i++){
    const attempt=attempts[i];
    try{const {parsed,payload,toolCalls,durationMs,rate}=await callGroq({...attempt,prompt});const result=sanitizeResult(parsed,clientUrl,count,attempt.model,mode,{tool_calls:toolCalls,fallback_used:i>0,usage:payload?.usage||null,duration_ms:durationMs,rate});result.meta.returned_count=result.prospects.length;if(!result.prospects.length)throw new Error('Research returned zero usable Korean prospects');return Response.json(result,{headers:{'Cache-Control':'no-store'}})}catch(error){failures.push(safeError(error?.message||'Unknown Groq error'));if([401,403].includes(error?.status))return Response.json({error:failures.at(-1),hint:'Groq API authentication/permission failed.'},{status:502});if(error?.status===429)return Response.json({error:failures.at(-1),hint:`Groq rate limit reached. Wait ${error.retryAfter||'a few'} seconds and retry with 5 prospects in Fast mode.`,rate:error.rate||null},{status:429,headers:{'Cache-Control':'no-store'}})}
  }
  return Response.json({error:`Korea pipeline generation failed. ${failures.join(' | ')}`,hint:'Run diagnostics, then retry 5 prospects in Fast mode.'},{status:502,headers:{'Cache-Control':'no-store'}});
}
