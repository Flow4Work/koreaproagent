...
import { listSentCompanies, markCompanySent, matchSentCompanies } from '../lib/sent-companies.js';
...
async function oauthCallback(request) {
  ...
  if (!state || !returnedState || returnedState !== state.nonce || Number(state.exp || 0) < Date.now()) return redirectResponse(appRedirect(origin, returnTo, 'state_error'));
