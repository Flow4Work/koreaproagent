...
async function oauthCallback(request) {
  ...
  try {
    ...
    if (!tokens.access_token || !tokens.refresh_token) return redirectResponse(appRedirect(origin, returnTo, 'refresh_token_missing'));
    const email = await fetchGoogleEmail(tokens.access_token);
    if (!email || email !== config.senderEmail) return redirectResponse(appRedirect(origin, returnTo, 'wrong_account'));
...
async function sentCompanies(request) {
  ...
  const items = Array.isArray(payload.items) ? payload.items.slice(0, MAX_SENT_LOOKUP_ITEMS) : [];
  if (!items.length) return Response.json({ ok: true, sentIds: [] }, { headers: { 'Cache-Control': 'no-store' } });
  try {
...
async function sentDomains(request) {
  if (!sameOriginRequest(request)) return Response.json({ error: '허용되지 않은 요청입니다.' }, { status: 403 });
  const secret = gmailConfig(request).sessionSecret;
  if (!secret) return Response.json({ error: '발송 이력 보안 설정이 없습니다.' }, { status: 503 });
  try {
    const domains = await listSentCompanyDomains(secret, MAX_SENT_LOOKUP_ITEMS);
    return Response.json({ ok: true, domains }, { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
    console.error('sent-company domain list failed', String(error?.message || error).slice(0, 300));
    return Response.json({ error: '발송 완료 회사 목록을 불러오지 못했습니다.' }, { status: 502, headers: { 'Cache-Control': 'no-store' } });
  }
}

async function sentHistory(request) {
  if (!sameOriginRequest(request)) return Response.json({ error: '허용되지 않은 요청입니다.' }, { status: 403 });
  if (!gmailConfigured(request)) return Response.json({ error: 'Gmail 발송 설정이 아직 완료되지 않았습니다.' }, { status: 503 });
  const config = gmailConfig(request);
  const session = readSession(request, config.sessionSecret);
  if (!session?.refreshToken || session?.email !== config.senderEmail) return reconnectResponse('발송 이력을 보려면 Gmail을 연결해주세요.');
  try {
    const history = await listSentCompanies(config.sessionSecret, 20);
    return Response.json({ ok: true, ...history }, { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
    console.error('sent-company history failed', String(error?.message || error).slice(0, 300));
    return Response.json({ error: '발송 이력을 불러오지 못했습니다.' }, { status: 502, headers: { 'Cache-Control': 'no-store' } });
  }
}
...
async function send(request) {
  ...
  const html = String(payload.html || '').trim().slice(0, 30000);
  ...
  try {
    ...
    const raw = buildRawMessage({ to, subject, body,html,senderEmail:config.senderEmail,senderName:config.senderName });
    const sent = await sendGmailMessage(accessToken, raw);
    const sentAt = new Date().toISOString();
    const isTestMessage = testMode || /^\[TEST\]\s*/i.test(subject);
    let historySaved = isTestMessage;
    let historyWarning = '';
    if (!isTestMessage) {
      try {
        await markCompanySent(companyKey, config.sessionSecret, sentAt, { name: companyName });
        historySaved = true;
      } catch (error) {
        historySaved = false;
        historyWarning = '메일은 발송됐지만 발송 이력 저장에 실패했습니다.';
        console.error('sent-company save failed after Gmail success', String(error?.message || error).slice(0, 300));
      }
    }
    return Response.json({
      ok: true,
...
export async function GET(request) {
  ...
  if (action === 'sent-history') return sentHistory(request);
  return Response.json({ error: '지원하지 않는 Gmail 작업입니다.' }, { status: 400, headers: { 'Cache-Control': 'no-store' } });
}

export async function POST(request) {
  const action = new URL(request.url).searchParams.get('action') || 'send';
  if (action === 'sent-companies') return sentCompanies(request);
  if (action === 'sent-domains') return sentDomains(request);
  return send(request);
}
